...
(function() {
    ...
    for (var i = 0; i < buttons.length; i++) {
      ...
      if (name.indexOf('_save') !== -1 || 
        ...
        if (btn.value && btn.value.indexOf('(Saving...)') === -1) {
          btn.value = (btn.getAttribute('data-orig-val') || btn.value) + ' (Saving...)';
        }
      }
    }
  }
  ...
  document.addEventListener('submit', function(e) {
    ...
    var disabledButtons = form.querySelectorAll('input[type="submit"]:disabled, button[type="submit"]:disabled');
  ...
  }, false); // Use bubble phase so Django's handlers run first
