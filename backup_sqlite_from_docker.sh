...
docker cp $CONTAINER:$DB_PATH_IN_CONTAINER $BACKUP_DIR_ON_HOST/$FILENAME && echo "✅ Backup saved as $BACKUP_DIR_ON_HOST/$FILENAME" || echo "❌ Failed to create backup"
cp $BACKUP_DIR_ON_HOST/$FILENAME $DB_PATH_ON_HOST && echo "✅ Latest db saved as $DB_PATH_ON_HOST | $DATE" || echo "❌ Failed to copy latest db to $DB_PATH_ON_HOST | $DATE"
